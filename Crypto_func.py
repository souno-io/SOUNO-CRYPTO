...
def sc_bin2dec(cryptostr):
    print(str(int(cryptostr, 2)))
    return str(int(cryptostr, 2)).encode()


# 十进制 to 十六进制: hex()
def sc_dec2hex(cryptostr):
    num = int(cryptostr)
    mid = []
    while True:
        if num == 0: break
        ...
        mid.append(base[rem])
    return ''.join([str(x) for x in mid[::-1]]).encode()


# 十六进制 to 十进制
def sc_hex2dec(cryptostr):
    return str(int(cryptostr.upper(), 16)).encode()


# 二进制 to 八进制
def sc_bin2oct(cryptostr):
    return str(oct(int(sc_bin2dec(cryptostr).decode()))).encode()


# 二进制 to 十六进制: hex(int(str,2))
def sc_bin2hex(cryptostr):
    return sc_dec2hex(sc_bin2dec(cryptostr)).decode().encode()


# 十进制 to 二进制
def sc_dec2bin(cryptostr):
    return str(bin(int(cryptostr))).encode()


# 十进制 to 八进制
def sc_dec2oct(cryptostr):
    return str(oct(int(cryptostr))).encode()


# 十六进制 to 二进制
def sc_hex2bin(cryptostr):
    return sc_dec2bin(sc_hex2dec(cryptostr.upper()).decode()).decode().encode()


# 十六进制 to 八进制
def sc_hex2oct(cryptostr):
    return str(sc_dec2oct(int(sc_hex2dec(cryptostr).decode())).decode()).encode()


# 八进制 to 十进制
def sc_oct2dec(cryptostr):
    return str(int(cryptostr, 8)).encode()


# 八进制 to 二进制
def sc_oct2bin(cryptostr):
    return str(bin(int(cryptostr, 8))).encode()


# 八进制 to 十六进制
def sc_oct2hex(cryptostr):
    return str(hex(int(cryptostr, 8))).encode()


# 十六进制 to 字符串
def sc_hex2char(cryptostr):
    #    binascii.a2b_hex(hexstr)
    return binascii.unhexlify(cryptostr)


# 字符串 to 十六进制
def sc_char2hex(cryptostr):
    #    binascii.b2a_hex(cryptostr)
    return binascii.hexlify(cryptostr.encode())
