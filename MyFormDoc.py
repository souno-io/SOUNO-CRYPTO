...
class QmyFormDoc(QWidget):
    ...
    def createContextMenu(self):
        ...
        # 进制转换
        scale = self.menuList.addMenu("Scale进制")
        sc_bin2dec = scale.addAction("二进制转十进制")
        sc_bin2oct = scale.addAction("二进制转八进制")
        sc_bin2hex = scale.addAction("二进制转十六进制")
        sc_oct2bin = scale.addAction("八进制转二进制")
        sc_oct2dec = scale.addAction("八进制转十进制")
        sc_oct2hex = scale.addAction("八进制转十六进制")
        sc_dec2bin = scale.addAction("十进制转二进制")
        sc_dec2oct = scale.addAction("十进制转八进制")
        sc_dec2hex = scale.addAction("十进制转十六进制")
        sc_hex2bin = scale.addAction("十六进制转二进制")
        sc_hex2oct = scale.addAction("十六进制转八进制")
        sc_hex2dec = scale.addAction("十六进制转十进制")
        sc_hex2char = scale.addAction("十六进制转字符串")
        sc_char2hex = scale.addAction("字符串转十六进制")
        ...
        for i in Crypto_json['Plug']:
            for j in Crypto_json['Plug'][i]:
                exec("{crypto_name} = plug_{i}.addAction(\"{name}\")".format(crypto_name=j["crypto_name"], i=i,
                                                                             name=j["name"]))
                exec("{crypto_name}.triggered.connect(partial(self.trig_func,\"{crypto_name}\",{key},\"{i}\"))".format(
                    crypto_name=j["crypto_name"], key=j["key"], i=i))
        ...
        ##进制系列
        sc_bin2dec.triggered.connect(lambda: self.trig_func("sc_bin2dec", False, "scale"))
        sc_bin2hex.triggered.connect(lambda: self.trig_func("sc_bin2hex", False, "scale"))
        sc_bin2oct.triggered.connect(lambda: self.trig_func("sc_bin2oct", False, "scale"))
        sc_oct2bin.triggered.connect(lambda: self.trig_func("sc_oct2bin", False, "scale"))
        sc_oct2dec.triggered.connect(lambda: self.trig_func("sc_oct2dec", False, "scale"))
        sc_oct2hex.triggered.connect(lambda: self.trig_func("sc_oct2hex", False, "scale"))
        sc_dec2bin.triggered.connect(lambda: self.trig_func("sc_dec2bin", False, "scale"))
        sc_dec2oct.triggered.connect(lambda: self.trig_func("sc_dec2oct", False, "scale"))
        sc_dec2hex.triggered.connect(lambda: self.trig_func("sc_dec2hex", False, "scale"))
        sc_hex2bin.triggered.connect(lambda: self.trig_func("sc_hex2bin", False, "scale"))
        sc_hex2oct.triggered.connect(lambda: self.trig_func("sc_hex2oct", False, "scale"))
        sc_hex2dec.triggered.connect(lambda: self.trig_func("sc_hex2dec", False, "scale"))
        sc_hex2char.triggered.connect(lambda: self.trig_func("sc_hex2char", False, "scale"))
        sc_char2hex.triggered.connect(lambda: self.trig_func("sc_char2hex", False, "scale"))
